...
- OIDC Well Known endpoint: https://auth1.sb.moneff.dev/.well-known/openid-configuration
- baseUrl: https://auth1.sb.moneff.dev/

### Step by Step guide to connect to sandbox

#### Step 1: Pre-Requisites for TPP
Ensure that the following pre-requisites are met before onboarding onto Moneff.
...
response = requests.post("https://ob-api1.sb.moneff.dev/dynamic-client-registration/v3.1/register",
...
Once a TPP successfully registers to Moneff, they can download the environment file for their client via an Moneff helper api

```
Endpoint: https://ob-api1.sb.moneff.dev/ozone/v1.0/postman/environment
...
https://rs1.sb.moneff.dev
https://auth1.sb.moneff.dev/token
...
Once the PSU consent is successful, Moneff will redirect back to the redirect URI. Copy the Authcode from the URL
